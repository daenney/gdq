...
func New(ctx context.Context, client *http.Client) *GDQ {
	return &GDQ{
		ctx:  ctx,
		c:    client,
		base: trackerV1,
...
func (g *GDQ) Latest() (*Event, error) {
	body, err := getWithCtx(g.ctx, g.c, fmt.Sprintf("%s/search?type=event", g.base))
	if err != nil {
		return nil, err
	}
...
func (g *GDQ) GetSchedule(ev Event) (*Schedule, error) {
	grp, ctx := errgroup.WithContext(g.ctx)
	queries := []string{
		fmt.Sprintf("%s/search?type=run&event=%d", g.base, ev.ID),
		fmt.Sprintf("%s/search?type=runner&event=%d", g.base, ev.ID),
		fmt.Sprintf("%s/hosts/%d", g.base, ev.ID),
	}
	...
	for i, query := range queries {
		...
		grp.Go(func() error {
			...
			defer cancel()
			res, err := getWithCtx(ctx, g.c, query)
			if err == nil {
				results[i] = res
			}
			return err
